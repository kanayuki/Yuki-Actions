"""Stage 4: Rank -- classify proxies by country and generate country files.

Reads health.json, resolves GeoIP for unknown hosts, groups by country,
trims pools, and writes country/*.txt.
...
from .config import COUNTRY_DIR, Config, load_config
...
def rank(cfg: Config | None = None, state: StateManager | None = None) -> None:
    """Run Stage 4: GeoIP classify and generate country files."""
    ...
    # Filter to healthy, non-dormant links
    valid: list[LinkHealth] = [
        entry
        for entry in health.values()
        if not entry.dormant and entry.fail_count == 0 and entry.last_ok and entry.link
    ]
    ...
    small_countries = [
        ...
        if len(entries) < cfg.min_country_size and cc not in ("XX", "OTHER")
    ]
    if small_countries:
    ...
    for cc in sorted(by_country.keys()):
        ...
        avg_lat = (
            sum(e.latency_ms for e in entries) / len(entries) if entries else 0
        )
        grid.add_row(
            f"[bold]{cc}[/bold]", str(len(entries)), f"avg {avg_lat:.0f} ms"
        )
