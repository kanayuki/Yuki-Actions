"""xray-core engine — multi-port SOCKS proxy testing.

Starts a single xray process with N inbound/outbound pairs, then tests
each proxy via its dedicated SOCKS port using HTTP requests.
"""

from __future__ import annotations

import base64
import json
import logging
import shutil
import subprocess
import tempfile
import time
import urllib.parse
from concurrent.futures import ThreadPoolExecutor, as_completed
from pathlib import Path
from typing import Callable

import requests

from . import TestEngine, TestResult
from .binary import ensure_binary

logger = logging.getLogger(__name__)

_BASE_SOCKS_PORT = 20001
_SUPPORTED = {"vmess", "vless", "ss", "trojan"}


# ---------------------------------------------------------------------------
# Share link → xray outbound converter
# ---------------------------------------------------------------------------


def _b64decode(s: str) -> bytes:
    s = s.replace("-", "+").replace("_", "/")
    pad = 4 - len(s) % 4
    if pad != 4:
        s += "=" * pad
    return base64.b64decode(s)


def _strip_fragment(url: str) -> tuple[str, str]:
    idx = url.find("#")
    if idx == -1:
        return url, ""
    return url[:idx], urllib.parse.unquote(url[idx + 1 :])


def _stream_settings(params: dict, security: str = "") -> dict:
    """Build xray streamSettings from parsed URL params."""
    net = params.get("type", "tcp")
    sec = security or params.get("security", "none")
    ss: dict = {"network": net}

    if sec == "tls":
        tls: dict = {}
        sni = params.get("sni") or params.get("host") or ""
        if sni:
            tls["serverName"] = sni
        fp = params.get("fp") or ""
        if fp:
            tls["fingerprint"] = fp
        alpn = params.get("alpn") or ""
        if alpn:
            tls["alpn"] = alpn.split(",")
        ss["security"] = "tls"
        ss["tlsSettings"] = tls
    elif sec == "reality":
        reality: dict = {
            "publicKey": params.get("pbk", ""),
            "shortId": params.get("sid", ""),
        }
        sni = params.get("sni") or ""
        if sni:
            reality["serverName"] = sni
        fp = params.get("fp") or ""
        if fp:
            reality["fingerprint"] = fp
        ss["security"] = "reality"
        ss["realitySettings"] = reality

    if net == "ws":
        ws: dict = {"path": params.get("path") or "/"}
        host = params.get("host") or ""
        if host:
            ws["headers"] = {"Host": host}
        ss["wsSettings"] = ws
    elif net == "grpc":
        ss["grpcSettings"] = {"serviceName": params.get("serviceName") or ""}
    elif net == "h2":
        h2: dict = {"path": params.get("path") or "/"}
        host = params.get("host") or ""
        if host:
            h2["host"] = [host]
        ss["httpSettings"] = h2

    return ss


def _vmess_outbound(link: str, tag: str) -> dict | None:
    b64 = _strip_fragment(link[8:])[0].strip()
    try:
        cfg = json.loads(_b64decode(b64).decode())
    except Exception:
        return None
    server = str(cfg.get("add", "")).strip()
    uuid = str(cfg.get("id", "")).strip()
    try:
        port = int(cfg.get("port", 0))
    except (TypeError, ValueError):
        return None
    if not (server and port and uuid):
        return None

    net = str(cfg.get("net") or "tcp")
    sec = "tls" if cfg.get("tls") == "tls" else "none"

    # Build stream settings from vmess JSON fields
    params: dict[str, str] = {
        "type": net,
        "security": sec,
        "path": str(cfg.get("path") or "/"),
        "host": str(cfg.get("host") or ""),
        "sni": str(cfg.get("sni") or ""),
        "fp": str(cfg.get("fp") or ""),
    }

    return {
        "tag": tag,
        "protocol": "vmess",
        "settings": {
            "vnext": [
                {
                    "address": server,
                    "port": port,
                    "users": [
                        {
                            "id": uuid,
                            "alterId": int(cfg.get("aid") or 0),
                            "security": str(cfg.get("scy") or "auto"),
                        }
                    ],
                }
            ]
        },
        "streamSettings": _stream_settings(params, sec),
    }


def _vless_outbound(link: str, tag: str) -> dict | None:
    url, _ = _strip_fragment(link)
    try:
        parsed = urllib.parse.urlparse(url)
        uuid = parsed.username or ""
        server = parsed.hostname or ""
        port = parsed.port
        params = dict(urllib.parse.parse_qsl(parsed.query))
    except Exception:
        return None
    if not (server and port and uuid):
        return None

    flow = params.get("flow", "")
    user: dict = {"id": uuid, "encryption": "none"}
    if flow:
        user["flow"] = flow

    return {
        "tag": tag,
        "protocol": "vless",
        "settings": {
            "vnext": [{"address": server, "port": port, "users": [user]}]
        },
        "streamSettings": _stream_settings(params),
    }


def _ss_outbound(link: str, tag: str) -> dict | None:
    url, _ = _strip_fragment(link)
    try:
        parsed = urllib.parse.urlparse(url)
        host = parsed.hostname or ""
        port = parsed.port
        if host and port:
            userinfo = parsed.username or ""
            try:
                decoded = _b64decode(userinfo).decode()
                cipher, password = decoded.split(":", 1)
            except Exception:
                cipher = urllib.parse.unquote(userinfo)
                password = urllib.parse.unquote(parsed.password or "")
        else:
            b64_part = url[5:].split("#")[0].split("?")[0]
            decoded_str = _b64decode(b64_part).decode()
            at = decoded_str.rfind("@")
            if at == -1:
                return None
            userinfo_str, hostport = decoded_str[:at], decoded_str[at + 1 :]
            cipher, password = userinfo_str.split(":", 1)
            if ":" not in hostport:
                return None
            host, port_str = hostport.rsplit(":", 1)
            port = int(port_str)
    except Exception:
        return None
    if not (host and port and cipher and password):
        return None

    return {
        "tag": tag,
        "protocol": "shadowsocks",
        "settings": {
            "servers": [
                {
                    "address": host,
                    "port": port,
                    "method": cipher.lower(),
                    "password": password,
                }
            ]
        },
    }


def _trojan_outbound(link: str, tag: str) -> dict | None:
    url, _ = _strip_fragment(link)
    try:
        parsed = urllib.parse.urlparse(url)
        params = dict(urllib.parse.parse_qsl(parsed.query))
        server = parsed.hostname or ""
        port = parsed.port
        password = urllib.parse.unquote(parsed.username or "")
    except Exception:
        return None
    if not (server and port and password):
        return None

    out: dict = {
        "tag": tag,
        "protocol": "trojan",
        "settings": {
            "servers": [{"address": server, "port": port, "password": password}]
        },
        "streamSettings": _stream_settings(params, params.get("security", "tls")),
    }
    return out


_CONVERTERS = {
    "vmess": _vmess_outbound,
    "vless": _vless_outbound,
    "ss": _ss_outbound,
    "trojan": _trojan_outbound,
}


def link_to_xray_outbound(link: str, tag: str) -> dict | None:
    link = link.strip()
    if "://" not in link:
        return None
    scheme = link.split("://", 1)[0].lower()
    conv = _CONVERTERS.get(scheme)
    if conv is None:
        return None
    try:
        return conv(link, tag)
    except Exception:
        return None


# ---------------------------------------------------------------------------
# Engine
# ---------------------------------------------------------------------------


class XrayEngine(TestEngine):
    _bin_path: Path | None = None

    @classmethod
    def name(cls) -> str:
        return "xray"

    @classmethod
    def supported_protocols(cls) -> set[str]:
        return _SUPPORTED

    @classmethod
    def is_available(cls) -> bool:
        from ..config import load_config
        cfg = load_config()
        path = ensure_binary("xray", cfg.xray_bin)
        if path:
            cls._bin_path = path
            return True
        return False

    def test_batch(
        self,
        links: list[str],
        *,
        timeout_ms: int = 6000,
        concurrency: int = 50,
        test_url: str = "http://www.gstatic.com/generate_204",
        on_done: Callable[[TestResult], None] | None = None,
    ) -> list[TestResult]:
        if not self._bin_path:
            self.is_available()
        if not self._bin_path:
            return [TestResult(link=l, ok=False, error="xray_unavailable") for l in links]

        results: list[TestResult] = []

        # Process in chunks of `concurrency` (each chunk = 1 xray process)
        for chunk_start in range(0, len(links), concurrency):
            chunk = links[chunk_start : chunk_start + concurrency]
            chunk_results = self._test_chunk(chunk, timeout_ms, concurrency, test_url, on_done)
            results.extend(chunk_results)

        return results

    def _test_chunk(
        self,
        links: list[str],
        timeout_ms: int,
        concurrency: int,
        test_url: str,
        on_done: Callable[[TestResult], None] | None,
    ) -> list[TestResult]:
        """Test one chunk via a single xray process with multiple SOCKS ports."""
        assert self._bin_path is not None

        # Build xray config
        inbounds: list[dict] = []
        outbounds: list[dict] = []
        rules: list[dict] = []
        port_to_link: dict[int, str] = {}
        unconvertible: list[str] = []

        for i, link in enumerate(links):
            tag = f"proxy-{i}"
            socks_tag = f"socks-{i}"
            socks_port = _BASE_SOCKS_PORT + i

            outbound = link_to_xray_outbound(link, tag)
            if outbound is None:
                unconvertible.append(link)
                continue

            inbounds.append({
                "tag": socks_tag,
                "port": socks_port,
                "listen": "127.0.0.1",
                "protocol": "socks",
                "settings": {"auth": "noauth"},
            })
            outbounds.append(outbound)
            rules.append({"inboundTag": [socks_tag], "outboundTag": tag})
            port_to_link[socks_port] = link

        results: list[TestResult] = []
        for link in unconvertible:
            r = TestResult(link=link, ok=False, error="xray_convert_failed")
            results.append(r)
            if on_done:
                on_done(r)

        if not outbounds:
            return results

        xray_cfg = {
            "log": {"loglevel": "none"},
            "inbounds": inbounds,
            "outbounds": outbounds,
            "routing": {"rules": rules},
        }

        cfg_dir = Path(tempfile.mkdtemp(prefix="xray-cfg-"))
        cfg_file = cfg_dir / "config.json"
        cfg_file.write_text(json.dumps(xray_cfg, ensure_ascii=False), encoding="utf-8")

        proc = subprocess.Popen(
            [str(self._bin_path), "run", "-c", str(cfg_file)],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )

        try:
            # Wait for xray to start
            time.sleep(1.0)
            if proc.poll() is not None:
                logger.error("xray exited early with code %d", proc.returncode)
                for link in port_to_link.values():
                    r = TestResult(link=link, ok=False, error="xray_start_failed")
                    results.append(r)
                    if on_done:
                        on_done(r)
                return results

            # Test each proxy via its SOCKS port
            timeout_s = timeout_ms / 1000.0
            with ThreadPoolExecutor(max_workers=min(concurrency, len(port_to_link))) as pool:
                futures = {
                    pool.submit(
                        self._test_via_socks, port, test_url, timeout_s
                    ): port
                    for port in port_to_link
                }
                for future in as_completed(futures):
                    port = futures[future]
                    link = port_to_link[port]
                    try:
                        latency = future.result()
                        r = TestResult(link=link, ok=True, latency_ms=round(latency, 1))
                    except Exception as e:
                        r = TestResult(link=link, ok=False, error=str(e))
                    results.append(r)
                    if on_done:
                        on_done(r)
        finally:
            proc.terminate()
            try:
                proc.wait(timeout=5)
            except subprocess.TimeoutExpired:
                proc.kill()
            shutil.rmtree(cfg_dir, ignore_errors=True)

        return results

    @staticmethod
    def _test_via_socks(port: int, test_url: str, timeout_s: float) -> float:
        """HTTP GET through a SOCKS5 proxy, return latency in ms."""
        proxies = {
            "http": f"socks5h://127.0.0.1:{port}",
            "https": f"socks5h://127.0.0.1:{port}",
        }
        start = time.monotonic()
        resp = requests.get(test_url, proxies=proxies, timeout=timeout_s, verify=False)
        latency = (time.monotonic() - start) * 1000.0
        if resp.status_code not in (200, 204):
            raise RuntimeError(f"HTTP {resp.status_code}")
        return latency
